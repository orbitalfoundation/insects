#!/usr/bin/env bash
# Runs ON the insect VM (installed by deploy/setup-autodeploy.sh, fired by a
...
REPO_URL="https://github.com/orbitalfoundation/insects.git"
BRANCH="main"
BASE="/srv/autodeploy"
REPO="$BASE/repo"
...
if [ -f deploy/Caddyfile ] && ! cmp -s deploy/Caddyfile /srv/Caddyfile 2>/dev/null; then
  ...
  docker exec bug caddy reload --config /etc/caddy/Caddyfile --adapter caddyfile 2>/dev/null \
    || docker restart bug >/dev/null
