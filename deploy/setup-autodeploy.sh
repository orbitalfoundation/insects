...
#   deploy/setup-autodeploy.sh [vm-name]      (default: insect)
#
...
SSH 'sudo tee /etc/systemd/system/insect-autodeploy.service >/dev/null' <<'UNIT'
[Unit]
Description=Insects — rebuild + redeploy when GitHub main changes
...
SSH 'sudo tee /etc/systemd/system/insect-autodeploy.timer >/dev/null' <<'UNIT'
[Unit]
Description=Poll GitHub main for Insect deploys
...
SSH 'sudo systemctl daemon-reload && sudo systemctl enable --now insect-autodeploy.timer'

echo "› first run (deploys current main now)"
SSH 'sudo systemctl start insect-autodeploy.service'
SSH 'systemctl status insect-autodeploy.timer --no-pager -l | head -6'

echo "✓ auto-deploy installed. Watch it with:"
echo "    ssh $DEST journalctl -u insect-autodeploy -f"
