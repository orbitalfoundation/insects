#!/usr/bin/env bash
# One-time provisioning for the Insect VM on exe.dev. Idempotent — safe to re-run;
...
# Usage: deploy/provision.sh [vm-name]      (default: insect)
set -euo pipefail
...
$SSH "$DEST" "sudo systemctl enable --now docker >/dev/null 2>&1; \
  docker rm -f bug 2>/dev/null || true; \
  docker run -d --name bug --restart unless-stopped \
    -p $PORT:80 \
